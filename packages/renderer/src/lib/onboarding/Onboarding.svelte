...
{#if activeStep}
  <OnboardingWizardShell
    ...
    class={globalOnboarding ? 'fixed inset-0 z-45 bg-[var(--pd-content-card-bg)]' : 'bg-[var(--pd-content-card-bg)]'}>
    ...
    {#snippet rightContent()}
      {#if activeStep.step.component}
        <div class="min-w-[700px] mx-auto pt-4" aria-label="Onboarding Component">
    ...
    {#snippet footer()}
      {#if !activeStep.step.completionEvents || activeStep.step.completionEvents.length === 0}
        <div class="flex flex-col gap-4">
          {#if activeStep.step.state !== 'failed'}
            <div class="text-center text-sm text-[var(--pd-content-card-text)]" aria-label="Next Info Message">
              Press the <span class="text-[var(--pd-button-text)] bg-[var(--pd-button-primary-bg)] px-1.5 py-0.5 rounded-sm text-xs font-medium">Next</span> button
              below to proceed.
            </div>
          {:else}
            <div class="text-center text-sm text-[var(--pd-content-card-text)]" aria-label="Exit Info Message">
